...
const putCustomer = async (id, customer) => {
	...
	const sql =
		"UPDATE clientes SET nome=$1, idade=$2, uf=$3, id=$4 WHERE id=$5";

	const values = [
		customer.nome, // Vou deixar o db passar com a restrição de nome n pode ser nulo
		customer.idade !== undefined ? customer.idade : null,
		customer.uf !== undefined ? customer.uf : null,
		customer.id !== undefined ? customer.id : id,
		id,
	];
